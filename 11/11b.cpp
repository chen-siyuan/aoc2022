...
int main() {
    ...
    int n = 8, m = 10000;
    int constexpr MOD = 2 * 3 * 5 * 7 * 11 * 13 * 17 * 19;
    ...
    monkeys[6].operation = [&](int x) -> int { return int(ll(x) * x % MOD); };
    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            while (!monkeys[j].items.empty()) {
                int item = monkeys[j].items.front() % MOD;
                monkeys[j].items.pop_front();
                item = monkeys[j].operation(item) % MOD;
